 ...
 * v1 families: resumeSubheading | tabularx-itemize | roleheading
 *
 * `roleheading` is the main.tex-design macro family used by
 * templates/cv-template.tex (and user hand-tuned resumes like data/main.tex):
 * \\roleheading/\\projheading headings, \\bul{...} bullets inside
 * \\bullist/\\stopbulls, and a skills section of `\\textbf{Category:} items \\`
 * lines. Detection must run BEFORE the tabularx-itemize fallback because the
 * roleheading documents load tabularx and use itemize as well.
 */

import { escapeLatex } from './latex-escape.mjs';

export const SUPPORTED_FAMILIES = ['resumeSubheading', 'tabularx-itemize', 'roleheading'];

export const UNSUPPORTED_HINT =
  'Unsupported LaTeX CV layout. v1 supports \\resumeSubheading + \\resumeItem macros, ' +
  'the \\roleheading/\\bul family (main.tex design), or tabularx + itemize without resume macros. ' +
...
export function detectFamily(tex) {
  ...
  // Checked before the tabularx fallback: roleheading documents load
  // tabularx and use itemize too, so the looser check would steal them.
  if (/\\roleheading\b/.test(tex) && /\\bul\b/.test(tex) && /\\bullist\b/.test(tex)) {
    return 'roleheading';
  }
...
/**
 * Index range of a section body by (case-insensitive) title, searching from
 * the document body onward. Returns {start, end} offsets into `tex`, or null
 ...
 */
function sectionBodyBounds(tex, titles) {
  const body = bodyStart(tex);
  const alt = titles.join('|');
  const re = new RegExp(String.raw`\\section\{[^}]*` + alt + String.raw`[^}]*\}`, 'i');
  const m = re.exec(tex.slice(body));
  if (!m) return null;
  const start = body + m.index + m[0].length;
  const next = tex.slice(start).match(/\\section\{/);
  const end = next ? start + next.index : tex.length;
  return { start, end };
}

/**
 * Extract the roleheading-family skill lines as slots. The family writes
 * skills as `\\textbf{Category:} items \\` — the colon sits INSIDE the bold
 * label and the items are plain text (not a braced group), unlike the
 * `\\textbf{Cat}{: items}` shape extractSkillValues() handles.
 *
 * Only the skills section is scanned: roleheading documents also use the
 * same shape for `\\textbf{Coursework:}` / `\\textbf{Honors \& Activities:}`
 * lines under Education, which are NOT editable skills (their values are
 * wrapped in \\textit, so patching them as plain text would destroy markup).
 *
 * @param {string} tex
 * @returns {Array<{kind: string, text: string, span: {start: number, end: number}}>}
 */
function extractRoleheadingSkillValues(tex) {
  const bounds = sectionBodyBounds(tex, ['technical skills', 'skills']);
  if (!bounds) return [];

  const slots = [];
  let searchFrom = bounds.start;
  while (searchFrom < bounds.end) {
    const idx = tex.indexOf('\\textbf{', searchFrom);
    ...
    const category = tex.slice(openCat + 1, closeCat);
    if (!/:$/.test(category.trim())) continue; // not a "Category:" label

    // Scan to the end of the line: an unescaped LaTeX line break (`\\`) or
    // newline, skipping other escaped characters.
    let i = closeCat + 1;
    let lineEnd = -1;
    while (i < bounds.end) {
      if (tex[i] === '\\') {
    ...
    if (lineEnd === -1) lineEnd = bounds.end;

    // Slot span covers exactly the items text (trimmed), so a no-op patch
    // with escape:false reproduces the document byte-for-byte.
    let start = closeCat + 1;
    while (start < lineEnd && /\s/.test(tex[start])) start++;
    let end = lineEnd;
    while (end > start && /\s/.test(tex[end - 1])) end--;
    if (end > start) {
      slots.push({
        kind: 'skill',
...
export function extractSlots(tex, family) {
  ...
  if (family === 'roleheading') {
    ...
    const bullets = dedupe(
      extractMacroBodies(tex, 'bul', 'bullet').sort(byPosition),
    ).map((s, i) => ({ ...s, id: `bullet-${i}` }));
    const skills = dedupe(
      extractRoleheadingSkillValues(tex).sort(byPosition),
    ).map((s, i) => ({ ...s, id: `skill-${i}` }));
    return [...bullets, ...skills];
  }
  return [];
}
