...
export const SUPPORTED_FAMILIES = ['resumeSubheading', 'roleheading', 'tabularx-itemize'];
...
/**
 * @param {string} tex
 * @returns {'resumeSubheading'|'roleheading'|'tabularx-itemize'|null}
 */
export function detectFamily(tex) {
  ...
  // roleheading is checked before tabularx-itemize: roleheading documents load
  // tabularx and use itemize as well (modes/latex-tex.md), so an unpatched
  // document would otherwise be misdetected and only its `\item` lines extracted.
  if (/\\roleheading\b/.test(tex) && /\\bul\{/.test(tex) && /\\bullist\b/.test(tex)) {
    return 'roleheading';
  }
...
/**
 * Extract `\textbf{Category:} items \\` skill lines within the document's
 * Skills section only (Education's `Coursework:`/`Honors & Activities:` lines
 * share the shape but are not editable skills — modes/latex-tex.md).
 *
 * @param {string} tex
 * @returns {Array<{id: string, kind: string, text: string, span: {start: number, end: number}}>}
 */
function extractRoleheadingSkillValues(tex) {
  const docStart = bodyStart(tex);
  const body = tex.slice(docStart);
  // Scope to the Skills section: from \section{...Skills...} to the next
  // \section (or end of body).
  const secRe = /\\section\{([^}]*)\}/g;
  let skillsStart = -1;
  let skillsEnd = body.length;
  let m;
  while ((m = secRe.exec(body)) !== null) {
    if (/skills/i.test(m[1])) {
      skillsStart = m.index + m[0].length;
    } else if (skillsStart !== -1) {
      skillsEnd = m.index;
      break;
    }
  }
  ...
  const section = body.slice(skillsStart, skillsEnd);
  const slots = [];
  const textbfRe = /\\textbf\{([^{}]*)\}/g;
  while ((m = textbfRe.exec(section)) !== null) {
    if (isCommentedAt(section, m.index)) continue;
    // The family shape requires the category to end with ':'.
    if (!/:\s*$/.test(m[1])) continue;
    const itemsFrom = m.index + m[0].length;
    // Items run to the trailing `\\` line break (or end of the line).
    const lineEnd = section.indexOf('\n', itemsFrom);
    const limit = lineEnd === -1 ? section.length : lineEnd;
    let itemsTo = section.indexOf('\\\\', itemsFrom);
    if (itemsTo === -1 || itemsTo > limit) itemsTo = limit;
    const raw = section.slice(itemsFrom, itemsTo);
    const text = raw.trim();
    if (!text) continue;
    ...
    slots.push({
      ...
      text,
      span: {
        start: docStart + skillsStart + itemsFrom + leadWs,
        end: docStart + skillsStart + itemsFrom + leadWs + text.length,
      },
    });
  }
  return slots;
...
/**
 * @param {string} tex
 * @param {'resumeSubheading'|'roleheading'|'tabularx-itemize'} family
 * @returns {Array<{id: string, kind: string, text: string, span: {start: number, end: number}}>}
 */
export function extractSlots(tex, family) {
  if (family === 'roleheading') {
    const byPosition = (a, b) => a.span.start - b.span.start;
    const bullets = extractMacroBodies(tex, 'bul', 'bullet')
      .sort(byPosition)
      .map((s, i) => ({ ...s, id: `bullet-${i}` }));
    const skills = extractRoleheadingSkillValues(tex);
